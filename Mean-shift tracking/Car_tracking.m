%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%           MEAN SHIFT TRACKING - Scale adaptation
% ------------------------------------------------------------------------
...
type   = 'jpg';         % type of file
fplace = './car';    % folder of the images (eg. car_ng, plane)
files  = dir(fplace);   % search de directory chosen
filename0 = strcat(fplace,filesep,files(4).name);   % get the file names

[m,n,k1] = size(imread(filename0));                 % for plane project

background = zeros(m,n);                        
fz = size(files,1);
Images = uint8(zeros(m,n,3,fz));
for k=4:fz-1
    filename=strcat(fplace,filesep,files(k).name);
    IN =imread(filename);
    Images(:,:,1,k-3)=im2uint8(IN);
    Images(:,:,2,k-3)=im2uint8(IN);
    Images(:,:,3,k-3)=im2uint8(IN);
end
if(k1==1)
...
for i=1:m
    for j=1:n
        background(i,j)=median(Images(i,j,1,:));
    end
end
